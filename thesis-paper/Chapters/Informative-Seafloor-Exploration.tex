...
\label{InformativeSeafloorExploration}
	...
	\label{InformativeSeafloorExploration:MCJPIE}
		...
		\label{InformativeSeafloorExploration:MCJPIE:Binary}
			...
			Further, define $I_{n_{U}} \subseteq I_{n_{S}}$, $n_{U} := \Vert I_{n_{U}} \Vert$, as the indices corresponding to the subset with the maximum amount of unique samples \eqref{Equation:UniqueIndexSet}.
			
			\begin{equation}
				I_{n_{U}} := \argmax_{\{I \subseteq I_{n_{S}} | {^{s}}\bvec{y}^{\star} \neq {^{t}}\bvec{y}^{\star}, \forall s, t \in I\}} \Vert I \Vert 
			...
			\begin{equation}
				\begin{aligned}
					\tilde{\bvec{p}}^{\star} &:= \bigg\{ \frac{1}{n_{S}}\mathbb{F}_{I_{n_{S}}}({^{s}}\bvec{y}^{\star}) \bigg\}_{s \in I_{n_{U}}} \\
