...
def main():
    ...
    n_track = 15
    X_s = np.random.uniform(test_range_min/2, test_range_max/2, size = (n_track, n_dims))
...
def keep_going_until_surprise(xq_abs_opt, learned_classifier, whitenparams, decision_boundary):
...
def path_linearised_entropy_model(theta_add, r, x, memory, whitenparams):
...
def path_monte_carlo_entropy_model(theta_add, r, x, memory, whitenparams,
    n_draws = 1000, S = None):
...
def go_optimised_path(theta_add_init, x, r, memory, whitenparams, ranges,
    ...
    try:
        ...
        if objective == 'LE':
            ...
            def objective(theta_add, grad):
                return path_linearised_entropy_model(theta_add, r, x, memory, whitenparams)
